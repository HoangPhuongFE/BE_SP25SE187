...
import { SubmissionPeriodService } from "../services/submissionPeriod.service";
...
export class SubmissionPeriodController {
  ...
  async updateSubmissionPeriod(req: Request, res: Response) {
    ...
    if (type && !["TOPIC", "CHECK-TOPIC", "REVIEW", "DEFENSE"].includes(type)) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({
        success: false,
        message: "Type phải là một trong các giá trị: TOPIC, CHECK-TOPIC,REVIEW, DEFENSE",
      });
    }
