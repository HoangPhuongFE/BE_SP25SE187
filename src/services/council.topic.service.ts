...
export class CouncilTopicService {
  async createCouncil(data: {
    ...
    relatedSubmissionPeriodId?: string; // Thêm vào interface
    createdBy: string;
    startDate: Date;
    endDate: Date;
    status?: string;
    ...
    try {
      ...
      const newCouncil = await prisma.council.create({
        data: {
          ...
          relatedSubmissionPeriodId: data.relatedSubmissionPeriodId || null, // Thêm vào đây
  ...
  async deleteTopicCouncil(councilId: string, userId: string, ipAddress?: string): Promise<{ success: boolean; status: number; message: string; data?: any }> {
    try {
      ...
      if (!isAuthorized) throw new Error('Chỉ admin hoặc academic_officer mới có quyền xóa hội đồng');

      const council = await prisma.council.findUnique({ where: { id: councilId, isDeleted: false } });
      if (!council) {
        await prisma.systemLog.create({
          data: { userId, action: 'DELETE_TOPIC_COUNCIL_ATTEMPT', entityType: 'Council', entityId: councilId, description: 'Thử xóa hội đồng topic nhưng không tìm thấy hoặc đã bị đánh dấu xóa', severity: 'WARNING', ipAddress: ipAddress || 'unknown' },
        });
        return { success: false, status: HTTP_STATUS.NOT_FOUND, message: COUNCIL_MESSAGE.COUNCIL_NOT_FOUND };
      }
  ...
  async reviewTopicByCouncilMember(topicId: string, status: 'APPROVED' | 'REJECTED' | 'IMPROVED', userId: string, reviewReason?: string) {
    try {
      ...
      const updatedTopic = await prisma.topic.update({
        where: { id: topicId },
        data: { status, reviewReason: reviewReason || null, updatedAt: new Date() },
        include: { group: true },
      });
